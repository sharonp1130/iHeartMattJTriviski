...
abstract class Location implements ActiveRecordInterface
{
     ...
     * @return $this|\Location The current object (for fluent API support)
     */
    public function setLocationUser($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
        ...
        if ($this->user !== $v) {
            $this->user = $v;
            $this->modifiedColumns[LocationTableMap::COL_USER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : LocationTableMap::translateFieldName('LocationUser', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(LocationTableMap::COL_USER)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setLocationUser($arr[$keys[1]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LocationTableMap::COL_USER)) {
            $criteria->add(LocationTableMap::COL_USER, $this->user);
        }
        if ($this->isColumnModified(LocationTableMap::COL_LONGITUDE)) {
    ...
    {
        if ($v === null) {
            $this->setLocationUser(NULL);
        } else {
            $this->setLocationUser($v->getUserid());
        }
